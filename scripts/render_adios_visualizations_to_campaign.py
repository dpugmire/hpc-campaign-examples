...
def _import_hpc_campaign():
    """Import the installed hpc-campaign package with a useful error message."""
    try:
        ...
        if exc.name in {"hpc_campaign", "hpc_campaign.info", "hpc_campaign.manager"}:
            raise SystemExit(
                "Could not import hpc-campaign. Install this example's Python dependencies first."
            ) from exc
        raise
...
def resolve_requested_variable(bp_path: Path, variable: str, vis_type: str) -> str:
    """Resolve a logical request to the physical ADIOS name in one run.

    Ensemble members may prepend solver-specific strings such as ``hll_`` to
    every ADIOS variable. Streamline requests are synthetic targets handled by
    their own multi-variable resolver and therefore pass through unchanged.
    """
    if normalize_vis_type(vis_type) == "streamlines":
        return variable

    available = set(discover_variables(bp_path))
    resolved = resolve_var_name(available, variable)
    if resolved is None:
        preview = ", ".join(sorted(available)[:20])
        raise MissingVariableError(
            f"Variable {variable!r} not found in {bp_path}. Available variables include: {preview}"
...
def visualization_semantic_kwargs(
    variable: str,
    vis_type: str,
    available_variables: set[str] | None = None,
) -> dict[str, Any]:
    """Map rendered data to semantic arguments used by Manager.visualization()."""
    normalized_vis_type = normalize_vis_type(vis_type)
    if normalized_vis_type == "streamlines":
        target = normalize_streamline_target(variable)
        if target is not None and available_variables is not None:
            spec = STREAMLINE_TARGETS[target]
            components = [resolve_var_name(available_variables, name) for name in spec["components"]]
            if all(components):
                semantic: dict[str, Any] = {"streamline_by": tuple(components)}
                background = resolve_var_name(available_variables, str(spec["background"]))
                if background is not None:
                    semantic["color_by"] = background
                return semantic
...
def add_visualization(
    ...
    try:
        return manager.visualization(
            ...
            # In-memory PNG bytes have no external replica path and therefore
            # must be embedded. Paths under image_workdir are intentionally
            # registered as external replicas instead.
            store=image_workdir is None,
            metadata={
            ...
            replace=replace,
            **(semantic_kwargs or visualization_semantic_kwargs(variable, vis_type)),
        )
    finally:
        if image_workdir is not None:
            os.chdir(cwd)
