...
DEFAULT_SCHEMA_PATH = REPO_ROOT / "schemas" / "mhd_orszag_tang.yaml"
...
def apply_and_validate_schema(manager: Any, schema_path: Path) -> dict[str, Any]:
    """Embed the selected schema and verify it against registered dataset names."""
    resolved_schema = schema_path.expanduser().resolve()
    manager.set_schema(resolved_schema)
    layout = manager.validate_schema()

    instances = layout.get("instances")
    if instances is None:
        print("[ok] schema validated at campaign root")
    else:
        print(f"[ok] schema validated for {len(instances)} run(s)")
    return layout
